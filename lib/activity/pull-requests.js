...
async function pullRequestEvent(context, subscription, slackWorkspace) {
  let pullRequest = {
  ...
  const allEnabled = subscription.settings.pulls === 'all';

  if (!isDraft || allEnabled) {
...
async function updateSlackMessage(context, message, prMessage, slackWorkspace) {
...
async function onStatusOrCheckRun(context, subscription, slackWorkspace) {
  let sha;
  let id;
  ...
  await updateSlackMessage(context, message, prMessage, slackWorkspace);
}
