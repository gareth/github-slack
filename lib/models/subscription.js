...
const { labelValuePattern, parseFeatures } = require('../settings-helper');
...
module.exports = (sequelize, DataTypes) => {
  ...
  Object.assign(Subscription.prototype, {
    ...
    enableFeatures(features) {
      // TODO change this once we support multiple filters of the same type.
      const multipleFiltersAllowed = false;
      ...
      Object.entries(features).forEach(([name, value]) => {
        ...
        if (Array.isArray(DefaultSettings[name])) {
          ...
          if (multipleFiltersAllowed) {
            ...
            this.settings[name] = [value[value.length - 1]];
          }
        } else {
      ...
      return Object.keys(settings)
        .map((setting) => {
          if (settings[setting] === true) {
            ...
            // custom handler to make the artificial pseudo array look nice
            if (setting === 'required_labels') {
              const asString = settings[setting][0];
              return `required-label: '${asString}'`;
            } else {
              // handle array settings like list of required-labels
